...
# Structure for storing sprite data in logs
...
# Structure for storing sprite group data in logs
class GroupInfo(TypedDict):
    count: int
    sprites: list[SpriteInfo]


# Export only these functions when using "from logger import *"
__all__ = ["log_state", "log_event"]

# Expected game FPS
_FPS = 60

# Stop logging after this many seconds
_MAX_SECONDS = 16

# Maximum sprites to save per group snapshot
_SPRITE_SAMPLE_LIMIT = 10

# Track current frame count
_frame_count = 0

# Track whether log files were already created
...
def log_state() -> None:
    ...
    # Stop logging after max time limit
    if _frame_count > _FPS * _MAX_SECONDS:
        return

    # Save state snapshot about once per second
    _frame_count += 1
    ...
    # Get previous frame (caller function)
    frame_back = frame.f_back
    if frame_back is None:
        return

    # Copy local variables from caller scope
    ...
    for key, value in local_vars.items():
        ...
        if hasattr(value, "__class__") and "Group" in value.__class__.__name__:
            ...
            for i, sprite in enumerate(value):
                ...
                # Save sprite velocity if available
                ...
                # Save sprite radius if available
                if hasattr(sprite, "radius"):
                    sprite_info["rad"] = sprite.radius

                # Save sprite rotation if available
        ...
        # Save standalone sprite objects
    ...
    # Create new log file on first write
...
def log_event(event_type: str, **details: object) -> None:
    ...
    # Create new event log file on first write
